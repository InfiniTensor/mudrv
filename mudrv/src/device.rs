...
impl Device {
    ...
    pub fn new(index: c_int) -> Self {
        let mut device = 0;
        mudrv!(muDeviceGet(&mut device, index));
        Self(device)
    }
    ...
    pub fn alignment(&self) -> usize {
        self.get_attribute(MU_DEVICE_ATTRIBUTE_TEXTURE_ALIGNMENT) as _
    }

    #[inline]
    pub fn warp_size(&self) -> usize {
        self.get_attribute(MU_DEVICE_ATTRIBUTE_WARP_SIZE) as _
    }

    #[inline]
    pub fn sm_count(&self) -> usize {
        self.get_attribute(MU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT) as _
    }

    pub fn max_grid_dims(&self) -> Dim3 {
        Dim3 {
            x: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_X) as _,
            y: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Y) as _,
            z: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Z) as _,
        }
    }

    pub fn block_limit(&self) -> BlockLimit {
        BlockLimit {
            max_threads: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_BLOCK) as _,
            max_dims: Dim3 {
                x: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_BLOCK_DIM_X) as _,
                y: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_BLOCK_DIM_Y) as _,
                z: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_BLOCK_DIM_Z) as _,
            },
            max_smem: self
                .get_attribute(MU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK)
                .into(),
            max_smem_optin: self
                .get_attribute(MU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK_OPTIN)
                .into(),
            reserved_smem: self
                .get_attribute(MU_DEVICE_ATTRIBUTE_RESERVED_SHARED_MEMORY_PER_BLOCK)
                .into(),
            max_registers: self
                .get_attribute(MU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK)
                .into(),
        }
    }
    ...
    pub fn sm_limit(&self) -> SMLimit {
        SMLimit {
            max_blocks: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_BLOCKS_PER_MULTIPROCESSOR) as _,
            max_threads: self.get_attribute(MU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR)
                as _,
            max_smem: self
                .get_attribute(MU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_MULTIPROCESSOR)
                .into(),
            max_registers: self
                .get_attribute(MU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_MULTIPROCESSOR)
                .into(),
        }
    }
